...
import com.pos.rest.config.KafkaProperties;
...
class PosRestApiKafkaResourceIT {
    ...
    void setup() {
        ...
        PosRestApiKafkaResource kafkaResource = new PosRestApiKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/pos-rest-api-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/pos-rest-api-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
